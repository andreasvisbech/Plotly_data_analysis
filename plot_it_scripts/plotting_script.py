...
def plot_func(figure, graph_name, x_val, y_val, std_dev, marker, x_title, y_title, subplot_row, subplot_col, comment, i, param_dict, color_list, color_count, user_input_dict):
    ...
    if comment == 'None':
        ...
        if marker.lower() in ['lines', 'line']:
            ...
            if param_dict['error_marker'] == 'Error bands':
                ...
                figure.add_trace(
                    go.Scatter(
                        name=graph_name, x=x_val, y=y_val, mode='lines', legendgroup=graph_name, showlegend=legend_show, line=dict(width=param_dict['graph width'], color=color_list[color_count])),
                    row=subplot_row, col=subplot_col
                )
                
                figure.add_trace(
                    go.Scatter(
                        name=graph_name + '_upper', x=x_val, y=y_val_upper, mode='lines', legendgroup=graph_name, showlegend=False, line=dict(width=0, color=color_list[color_count])),
                    row=subplot_row, col=subplot_col
                )
                
                figure.add_trace(
                    go.Scatter(
                        name=graph_name + '_lower', x=x_val, y=y_val_lower, mode='lines', legendgroup=graph_name, showlegend=False, fillcolor=rgb_color, fill='tonexty',
            ...
            if param_dict['error_marker'] == 'Error bands':
                ...
                figure.add_trace(
                    go.Scatter(
                        name=graph_name, x=x_val, y=y_val, mode='markers', legendgroup=graph_name, showlegend=legend_show, marker=dict(size=param_dict['marker_size'], color=color_list[color_count])),
                    row=subplot_row, col=subplot_col
                )
        ...
        figure.add_trace(
            go.Bar(name=graph_name, x=x_val, y=y_val, legendgroup=graph_name, showlegend=legend_show, marker=dict(color=color_list[color_count])),
            row=subplot_row, col=subplot_col
        )
