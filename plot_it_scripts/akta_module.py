...
def akta_main_func(df, xs, ys, sample_idx, x_id, y_id, param_dict, master_dict, user_input_dict, plot_dict):
    ...
    plot_func(figure, graph_name, xs, ys, 'None', plot_marker, x_title, y_title, subplot_row, subplot_col, 'None', sample_idx, param_dict, color_list, color_count, user_input_dict)
    plot_func(plot_figure, graph_name, xs, ys, 'None', plot_marker, x_title, y_title, subplot_row, subplot_col, 'None', sample_idx, param_dict, color_list, color_count, user_input_dict)
    
    # Plotting the baseline values into interactive plot and the stativ "plotting plot"
    plot_func(figure, graph_name + '_baseline', baseline_values[0], baseline_values[1], 'None', 'lines', x_title, y_title, subplot_row, subplot_col, 'AKTA_baseline', sample_idx, param_dict, 'N/A', 'N/A', user_input_dict)
    plot_func(plot_figure, graph_name + '_baseline', baseline_values[0], baseline_values[1], 'None', 'lines', x_title, y_title, subplot_row, subplot_col, 'AKTA_baseline', sample_idx, param_dict, 'N/A', 'N/A', user_input_dict)
    
    # if a fraction has been specified by the user do calculations on this section
    if str(df['AKTA_fraction'][sample_idx]).count(';') == 1:
        ...
        plot_func(figure, graph_name + '_fraction', xsys_slice[x_id], xsys_slice[y_id], 'N/A', 'lines', x_title, y_title, subplot_row, subplot_col, 'AKTA_fraction', sample_idx, param_dict, color_list, color_count, user_input_dict)
        plot_func(plot_figure, graph_name + '_fraction', xsys_slice[x_id], xsys_slice[y_id], 'N/A', 'lines', x_title, y_title, subplot_row, subplot_col, 'AKTA_fraction', sample_idx, param_dict, color_list, color_count, user_input_dict)
    
    # If no fraction is specified 'N/A' values are appended to the master dict in to make sure value positions in the table are not shifted
    else:
